...
export default function DetailContent(props: EventData) {
    ...
    <>
      <div className="relative h-75 w-full">
        ...
        <p className="caption-1-medium absolute bottom-0 z-10 pl-5 text-gray-500">
          {props.mediaType} · {props.locationName}
        </p>
      </div>
      <div className="px-5">
        ...
        <div className="mt-8.5 flex flex-col gap-3">
          <div className="flex gap-2">
            ...
            <div className="flex flex-col gap-0.5">
              <p className="body-3-medium text-gray-200"> {props.username}</p>
              <p className="caption-1-regular text-gray-500">
                단관 경험 {props.recruitment}회 (상세정보)
              </p>
            </div>
          </div>
          <p className="body-2-medium text-gray-300">{props.eventTitle}</p>
          <p className="body-3-medium text-gray-600">{props.description}</p>
        </div>
        <div className="caption-1-regular mt-8 grid grid-cols-[74px_1fr] gap-y-2 rounded-xl bg-gray-950 px-5 pt-5 pb-6 text-gray-300">
          <span>예상 가격</span>
          <span>{props.estimatedPrice}</span>
          ...
          <span>모집 인원</span>
          <span>
            {props.minParticipants} - {props.maxParticipants}명
          </span>
        </div>
